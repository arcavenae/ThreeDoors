...
func parseCheckboxLineObsidian(line string) (text string, status core.TaskStatus, embeddedID string, isCheckbox bool) {
	type prefix struct {
		p string
		s core.TaskStatus
	}
	...
	return line, core.StatusTodo, "", false
}

// extractMetadata parses Obsidian metadata from task text and returns cleaned text.
func extractMetadata(text string) (cleaned string, tags []string, dueDate string, effort core.TaskEffort) {
	// Extract due date
...
// LoadTasks reads all Markdown files in the configured folder and parses checkbox core.
func (a *ObsidianAdapter) LoadTasks() ([]*core.Task, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *ObsidianAdapter) loadTasksLocked() ([]*core.Task, error) {
	dir := a.taskDir()
...
// parseFile reads a single Markdown file and extracts checkbox core.
func (a *ObsidianAdapter) parseFile(filePath string, now time.Time) ([]*core.Task, error) {
...
func (a *ObsidianAdapter) SaveTask(task *core.Task) error {
	a.mu.Lock()
	defer a.mu.Unlock()
...
// SaveTasks replaces all core. Writes tasks grouped by their source file.
// New tasks (no matching file) go to tasks.md.
func (a *ObsidianAdapter) SaveTasks(tasks []*core.Task) error {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *ObsidianAdapter) MarkComplete(id string) error {
	...
	for _, file := range files {
		...
		for i, line := range lines {
			...
			if !core.IsValidTransition(status, core.StatusComplete) {
...
func taskToObsidianLine(task *core.Task) string {
	var checkbox string
	switch task.Status {
