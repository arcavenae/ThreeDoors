...
| Name | File | Direction | Description |
|------|------|-----------|-------------|
| `textfile` | `text_file_provider.go` | Read/Write | YAML-based local file storage (default) |
| `applenotes` | `apple_notes_provider.go` | Bidirectional | macOS Notes integration via AppleScript |
| `obsidian` | `obsidian_adapter.go` | Bidirectional | Obsidian vault reader/writer |
| `jira` | `jira_provider.go` | Read (JQL filter) | Jira Cloud/Server via REST API |
| `github` | `github_provider.go` | Read | GitHub Issues via SDK |
| `reminders` | `reminders_provider.go` | Read/Write | macOS Reminders via JXA |
| `todoist` | `todoist_provider.go` | Read | Todoist via REST API |
