...
6. The display clearly labels them as "Door 1", "Door 2", and "Door 3", distributed horizontally from left to right.
7. The application responds to 'a' for the left door, 'w' for the middle/center door, and 'd' for the right door.
8. Pressing 's' re-rolls the doors, presenting a new set of three tasks.
9. The application still quits on 'q' or 'ctrl+c'.
...
*   **Door Display**: The three doors should be rendered horizontally, from left to right, to visually represent their selection via 'a', 'w', 'd' keys.
*   **Key Bindings**: Implement keyboard input handling for:
    *   'a': Selects the left door.
    *   'w': Selects the middle/center door.
    *   'd': Selects the right door.
    *   's': Re-rolls the doors, presenting a new set of three tasks.
...
- Added `SetHomeDir` to `file_manager.go` for testability.
- Created unit tests for `FileManager` in `internal/tasks/file_manager_test.go`.
- Updated `cmd/threedoors/main.go` to load tasks, select 3 randomly, and display them using `bubbletea` and `lipgloss`.
- Verified `q` and `ctrl+c` quit functionality.
- Manually verified reading from `~/.threedoors/tasks.txt` and its creation/modification.
...
- Modified `cmd/threedoors/main.go` to:
    - Update `model` struct.
    - Implement `initialModel` to load and select tasks.
    - Update `View` to render doors with `lipgloss` styling.
