...
- [x] Implement `MarkComplete()` with GraphQL mutation for state transition
- [x] Implement dynamic completed-state discovery per team
- [x] Cache team→completedStateId mapping with TTL
- [x] Integrate WALProvider wrapper for offline queuing
- [x] Implement `SaveTask()` for title/description updates
- [x] Add sync status reporting to `HealthCheck()`
- [x] Write unit tests for mutation construction
- [x] Write unit tests for WAL integration (queue, replay, dequeue)
- [x] Write integration test with mocked GraphQL server for full sync flow
