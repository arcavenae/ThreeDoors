...
- [ ] **Verify no out-of-scope files**: Review `git diff --stat` — ensure only files related to this story are changed
- [ ] **Verify AppleScript injection safety**: Ensure all note titles and task text passed to osascript are properly escaped (PR #17 had a security fix for AppleScript injection)
